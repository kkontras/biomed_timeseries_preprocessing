...
def load_edf_signalLabels(eeg_file):
...
def load_pyedf(eeg_file):
    ...
    for i in np.arange(f.signals_in_file):
        signals[f.getSignalHeader(i)["label"]] = f.readSignal(i)
        if f.getSignalHeader(i)["label"] == "Fp1":
            srate = f.getSignalHeaders()[i]['sample_rate']
    if not 'srate' in vars() :
        raise Exception("There is no Fp2 channel, you have to define here which label of channels we will trust for the sampling rate.")
    return signals, srate

def rearrange_headers(signals_dict, headers):
    ordered_signals = np.array([ signals_dict[i] for i in headers])
    return ordered_signals

def create_windows(signals, min_rate, window_secs = 30, overlap = 0.5):
    windows= np.array([signals[:,i-(min_rate*window_secs):i] for i in range((min_rate*window_secs),signals.shape[1],int(min_rate*window_secs*overlap))])
    return windows
