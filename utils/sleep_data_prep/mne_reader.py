...
class MNE_Reader():
    def __init__(self, config, edf_file, label_file, name, patient_num, file_num, mod_names):
        ...
        self.mne_file = mne.io.edf.edf.RawEDF(edf_file, verbose=30)
        self._file = edf_file
        self.config = config
        self.label_file = label_file
        self.patient_num = patient_num
        self.file_num = file_num
        self.headers = self.mne_file.ch_names
        self.ch_labels = [self.headers[i] for i in range(len(self.headers))]
        self.ch_dict = {}
        self.name = name
        self.exclude_map = []
        self.mod_names = mod_names
        self.srate = dict(self.mne_file.info)["sfreq"]
        self.file_duration = self.mne_file.n_times/self.srate
        ...
        self.save_agent = Get_Savior(save_type=self.config.save_type, dataset_name=self.config.dataset_type, savedir=self.config.save_dir,
            ...
            for p_label in possible_labels:
                for labels_index in range(len(self.ch_labels)):
                    if p_label == self.ch_labels[labels_index].lower():
                        ...
                        # break
        for channel in channel_labels:
            if channel not in self.ch_dict:
                self.ch_dict[channel] = -1
    ...
    def load_signals(self, ):
        ...
        for i, num_channels in enumerate(self.modalities):
            ...
            for ch_idx in range(count, count + num_channels):
                self.signals[mod_name]["sampling_rate"] = 256
                self.signals[mod_name]["num_channels"] = num_channels
                if self.ch_dict[self.desire_channels[ch_idx]] == -1:
                    self.signals[mod_name][ch_idx - count] = np.array([-1])
                    continue
                self.signals[mod_name][ch_idx - count] = self.mne_file[self.ch_dict[self.desire_channels[ch_idx]]][0][0]
    ...
    def resample(self, to_srate):
        ...
        for modality in self.signals["mod_name"]:
            for ch in range(self.signals[modality]["num_channels"]):
                if len(self.signals[modality][ch])==1 and self.signals[modality][ch] == -1 : continue
    ...
    def create_windows(self, mean= None, std = None ,enhancement={"Gaussian":{"mean":0,"std":0.2,"times":5}}):
        ...
        metric= {"mean_time":{},"mean_sq_time":{},"sum_time":{},"count_labels":np.zeros(self.config.num_classes+1)}
        ...
        for modality in self.signals["mod_name"]:
            ...
            signals = {}
            for ch in range(self.signals[modality]["num_channels"]):
                if len(self.signals[modality][ch])<self.signals[modality]["sampling_rate"]*50: continue
                signals[ch] = np.array(self.signals[modality][ch])
            # signals = {ch : np.array(self.signals[modality][ch]) for ch in range(self.signals[modality]["num_channels"])}
            ...
            while( start_index + window_samples <= len_signal ):
                ...
                current_window_signal = {ch: signals[ch][int(start_index): int(start_index) + window_samples] if len(signals[ch]) != 1 else np.array([-1]) for ch in signals.keys()}
                ...
                if flag:
                    ...
                    for ch in range(self.config.ch_per_mod[modality]):
                        if ch in current_window_signal and len(current_window_signal[ch]) >= 2:
                    ...
                    overlap = self.config.overlap_on if True else self.config.overlap_off #!Change this for each dataset u need different overlap
    ...
    def create_sfft_windows(self,  mean, std, enhancement={"Gaussian":{"mean":0,"std":0.2, "times":5 }}, imp_label=1, hdf5_file = None):
        ...
        for modality in self.signals["mod_name"]:
            ...
            len_signal =len(signals[list(signals.keys())[0]]) if len(list(signals.keys()))>0 else 0
            flag = False #init flag
            ...
            #V2
            # signals_concat = np.concatenate([np.expand_dims(signals[ch], axis=0) for ch in signals])
            # f, t, Zxx_total = sg.stft(signals_concat, srate, nperseg=2*srate, noverlap=srate, boundary=None, nfft=self.config.nfft_points,window=sg.windows.hamming(2*srate))
            # sig_count = 0
            Zxx = {}
            for ch in signals:
                f, _, Zxx[ch] = sg.stft(signals[ch], srate, nperseg=2 * srate, noverlap=srate, boundary=None,
                                          nfft=self.config.nfft_points, window=sg.windows.hamming(2 * srate))

            # Zxx = {ch:np.asarray(eng.spectrogram(matlab.double(signals[ch].tolist()), matlab.double(eng.hamming(2*self.config.inner_stft_window*srate)), self.config.inner_stft_window*srate, self.config.nfft_points)) if len(signals[ch]) != 1 else np.array([-1]) for ch in range(self.signals[modality]["num_channels"])}
            # Zxx = {ch:np.asarray(eng.spectrogram(matlab.double(signals[ch].tolist()), matlab.double(eng.hamming(2*self.config.inner_stft_window*srate)), self.config.inner_stft_window*srate, self.config.nfft_points)) if len(signals[ch]) != 1 else np.array([-1]) for ch in range(self.signals[modality]["num_channels"])}

            for ch in Zxx.keys():
                if len(Zxx[ch]) != 1:
                    Zxx[ch] = np.abs(Zxx[ch])
                    Zxx[ch][Zxx[ch] == 0] = 1e-5
                    Zxx[ch] = 20 * np.log10(Zxx[ch])
                    if (Zxx[ch] != Zxx[ch]).any():
                        print(Zxx[ch].min())
                        print(np.isnan(Zxx[ch]).any())
            #
            if mean and std:
                for ch in range(self.signals[modality]["num_channels"]):
                    if len(self.signals[modality][ch]) != 1:
                        continue
                    assert isinstance(mean[modality][ch], (int, float)) and isinstance(std[modality][ch], (int, float))
                    signals[ch] = (signals[ch] - mean[modality][ch]) / (np.array(std[modality][ch]) + 1e-20)
            ...
            while ((start_index + window_samples)*self.config.inner_stft_window*srate <= len_signal):
                ...
                if flag:
                    if not flag_label:
                        ...
                        continue
                    overlap = self.config.overlap_on if not isinstance(w_label, np.ndarray) and w_label == 1 else self.config.overlap_off
                    for ch in Zxx:
                        if len(Zxx[ch]) != 1:
                            self.signals[modality]["windows_stft"][ch].append(np.array(Zxx[ch][:, int(start_index): int(start_index + window_samples -1)]))
            ...
            # if "starts" in self.signals[modality] and "starts_stft" in self.signals[modality]:
            #     for ch in range(len(self.signals[modality]["starts_stft"].keys())):
            #         for i in range(len(self.signals[modality]["starts_stft"][ch])):
            #
            #             if len(self.signals[modality]["starts_stft"][ch]) != len(self.signals[modality]["starts"][ch]): print(
            #                 "There is no exact alignment between time and stft views")
            #
            #             print(self.signals[modality]["starts_stft"][ch][i])
            #             print(self.signals[modality]["starts"][ch][i])
            #             if (np.array(self.signals[modality]["starts_stft"][ch][i]) != np.array(self.signals[modality]["starts"][ch][i])).any():
            #                 print(modality)
            #                 print("{}-{}".format(len(self.signals[modality]["starts_stft"][ch][i]),
            #                                      len(self.signals[modality]["starts"][ch][i])))
            #     print("There is no exact alignment between time and stft views")
            ...
            for ch in range(self.config.ch_per_mod[modality]):
                if len(self.signals[modality]["windows_stft"][ch])<1 : continue
